    ...
    <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="bg-white shadow rounded-lg p-8 space-y-6">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <select name="brand_id" class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-purple-400">
            ...
            <div>
                ...
                <select name="category_id" class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-purple-400">
            ...
            <div>
                ...
                <select name="gender_id" class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-purple-400">
            ...
            <div>
                ...
                <select name="product_type_id" class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-purple-400">
            ...
            <div class="flex flex-wrap gap-2">
                @foreach ($sizes as $size)
                    <label class="inline-flex items-center gap-2 border rounded px-3 py-1 cursor-pointer">
                        <input type="checkbox" name="sizes[]" value="{{ $size->id }}" {{ is_array(old('sizes')) && in_array($size->id, old('sizes')) ? 'checked' : '' }} />
                        <span>{{ $size->name }}</span>
                    </label>
                @endforeach
            </div>
        ...
        <div>
            ...
            <button type="button" id="addColorBtn" class="bg-purple-600 text-white px-4 py-2 rounded hover:bg-purple-700 transition">
                + Add Color
            </button>
        </div>
        ...
        <div class="text-right">
            <button type="submit" class="bg-purple-600 text-white px-6 py-2 rounded hover:bg-purple-700 transition">Add Product</button>
        </div>
    </form>
</div>
...
<script>
    ...
    addBtn.addEventListener('click', () => {
        ...
        div.innerHTML = `
            <select name="colors[${colorIndex}][color_id]" class="border px-2 py-1 rounded" required>
                <option value="">Select Color</option>
                ${colors.map(c => `<option value="${c.id}">${c.name}</option>`).join('')}
            </select>
            <input type="file" name="colors[${colorIndex}][images][]" multiple class="border px-2 py-1 rounded" required>
            <button type="button" class="removeColor bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600">-</button>
        `;
        container.appendChild(div);

        div.querySelector('.removeColor').addEventListener('click', () => div.remove());
